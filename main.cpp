...
int bitiShiftare[] = {1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1};
...
{
	bitset<48> expandareCheie;
	for(int i=0; i<48; ++i)
		expandareCheie[47-i] = R[32-E[i]];
	expandareCheie = expandareCheie ^ k;
	bitset<32> output;
	int x = 0;
	for(int i=0; i<48; i=i+6)
	{
		int row = expandareCheie[47-i]*2 + expandareCheie[47-i-5];
		int col = expandareCheie[47-i-1]*8 + expandareCheie[47-i-2]*4 + expandareCheie[47-i-3]*2 + expandareCheie[47-i-4];
...
void generareCheieRunda()
{
	...
	for(int round=0; round<16; ++round)
	{
		...
		for(int i=0; i<28; ++i)
			right[i] = realKey[i];
		left = leftShift(left, bitiShiftare[round]);
		right = leftShift(right, bitiShiftare[round]);
		for(int i=28; i<56; ++i)
			realKey[i] = left[i-28];
		for(int i=0; i<28; ++i)
			realKey[i] = right[i];
